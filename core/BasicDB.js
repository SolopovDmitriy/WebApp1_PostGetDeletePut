...
class BasicDB{
    ...
    getOneRow(searchPattern, callback){
        DBConnector.execute(function (conn){
            var _basa = conn.db(CONFIG.DB.BASENAME);    // ссылка на базу данных
            var curCollection = _basa.collection(this.getCollectionName());
            curCollection.findOne(searchPattern, function (err, result){
                conn.close();                
                //if(err) throw err;
                if(typeof callback === 'function'){
                    if(result) 
     ...
     */
    static updateRow(myquery = {}, newvalues={}, callback){
    ...
    /*
    //find(),             Находит документы, удовлетворяющие заданному критерию
    //findOne():          Находит один документ, удовлетворяющий заданному критерию
    //findById()          Находит документ по заданному идентификатору id
    //createCollection
    //listCollections().toArray     Находит и выводит полный список коллекций
    //collection('products').rename
    //collection('orders').drop
    //insertOne
    deleteOne
    updateOne
    findByIdAndRemove(), findByIdAndUpdate(), findOneAndRemove(), findOneAndUpdate():Находит один документ по id или по критерию, а затем или обновляет, или удаляет его
